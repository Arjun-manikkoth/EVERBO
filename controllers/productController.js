...
const productsLoad = async (req, res) => {
    try {
        ...
        const product = await Product.find({ is_deleted: 0 })
            .populate("category")
            .limit(limit)
            .skip((page - 1) * limit)
            .sort({ updatedAt: -1 });
        if (product) {
            ...
            res.render("products", { msg: PRODUCT_MESSAGES.NO_PRODUCTS });
        }
    } catch (error) {
...
const addProduct = async (req, res) => {
    try {
        ...
        if (Exists) {
            res.render("add_product", { message: PRODUCT_MESSAGES.PRODUCT_EXISTS, category });
        } else {
            ...
            if (productData) {
                ...
                const productData = await Product.findByIdAndUpdate(
                    { _id: data._id },
                    { $set: { subImages: subImagesArray } }
                );
                res.redirect("/admin/products");
            } else {
                res.render("add_product", {
                    message: PRODUCT_MESSAGES.PRODUCT_ADD_FAILED,
                    category,
                });
            }
        }
...
const editProductLoad = async (req, res) => {
    try {
        const id = req.query.id;
        const product = await Product.findOne({ _id: id, is_deleted: { $ne: 1 } }).populate(
            "category"
        );
...
const updateProduct = async (req, res) => {
    try {
        if (req.files) {
            ...
            const datas = await Product.findByIdAndUpdate(productId, {
                $set: { subImages: subImagesArray },
            });
            if (datas) {
                res.redirect("/admin/products");
            } else {
                res.render("edit_product", { message: PRODUCT_MESSAGES.PRODUCT_UPDATE_FAILED });
            }
        } else {
            const editData = await Product.updateOne(
                { _id: req.query.id },
                {
                    $set: {
                        name: req.body.name,
...
const updateProductThumb = async (req, res) => {
    try {
        const datas = await Product.findByIdAndUpdate(req.query.id, {
