...
const { ORDER_MESSAGES } = require("../constants/messages");
...
const orderLoad = async (req, res) => {
    try {
        ...
        const orderData = await Order.find({ grandTotalCost: { $exists: true, $ne: null } })
            .limit(limit)
            .skip((page - 1) * limit)
            .sort({ createdAt: -1 })
        ...
        if (orderData) {
            ...
            res.render("orders", { msg: ORDER_MESSAGES.NO_ORDERS, totalPages, currentPage: page });
        }
    } catch (error) {
