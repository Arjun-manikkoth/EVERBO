...
const editProfile = async (req, res) => {
    try {
        await User.findOneAndUpdate(
            { _id: req.query.id },
            { $set: { name: req.body.name, mobile_no: req.body.mobileno } }
        )
            .then(res.redirect("/profile"))
            .catch(console.log("Couldnt update user data"));
    } catch (error) {
        console.log(error.message);
    }
};
...
const cancelOrder = async (req, res) => {
    try {
        const orderData = await Order.findByIdAndUpdate(
            { _id: req.body.id },
            { $set: { reason: req.body.reason, orderStatus: "Cancelled" } },
            { new: true }
        ).populate("referralData.referredUser");
        ...
        orderData.cartData.forEach(async (prod) => {
            const productData = await Product.findByIdAndUpdate(
                { _id: prod.productId },
                { $inc: { quantity: prod.productQuantity } },
                { new: true }
            );
        });
        if (orderData) {
...
const orderReturn = async (req, res) => {
    try {
        const orderData = await Order.findByIdAndUpdate(
            { _id: req.body.id },
            { $set: { reason: req.body.reason, orderStatus: "Returned" } }
        ).populate("referralData.referredUser");
        const data = await User.findByIdAndUpdate(
            { _id: req.session.user_Id },
            {
...
const invoiceOrder = async (req, res) => {
    try {
        const data = await Order.findById({ _id: req.query.orderId, userId: req.session.user_Id })
            .populate("userId")
            .populate("addressChosen")
            .populate("cartData.productId");
        res.json(data);
...
const loadWallet = async (req, res) => {
    try {
        const walletData = await User.findById({ _id: req.session.user_Id }).populate(
            "wallet.walletTransaction.orderId"
        );
