...
const { ORDERS, COMMON_MESSAGES } = require("../constants/messages");
...
const salesDataLoad = async (req, res) => {
    try {
        ...
        let count = await Order.find({
            grandTotalCost: { $exists: true, $ne: null },
...
const downloadSalesPdf = async (req, res) => {
    try {
        ...
        res.status(500).send(COMMON_MESSAGES.INTERNAL_SERVER_ERROR);
    }
};
...
const loadDashboard = async (req, res) => {
    try {
        ...
        const totalUsers = await User.find({
            $and: [{ is_verified: 1 }, { is_blocked: 0 }],
        }).countDocuments();

        //Total Orders
        const totalOrders = await Order.find({
            $and: [{ orderStatus: "Delivered" }, { grandTotalCost: { $exists: true } }],
        }).countDocuments();
        ...
        const totalCategories = await Category.find({
            $and: [{ is_listed: true }, { is_deleted: 0 }],
        }).countDocuments();
        ...
        const weeklySalesData = await Promise.all(
            weeksInMonth.map(async (week, index) => {
